...
[[nodiscard]] bool get_module(DWORD pid, const char* name, std::uintptr_t& base, std::size_t& size) {
#undef Module32First
#undef Module32Next
#undef MODULEENTRY32
    ...
    if (Module32First(snap, &entry_32)) {
        do {
            ...
            if (string_equals_non_case_sensitive(entry_32.szExePath + last + 1, name)) {
                CloseHandle(snap);
                base = reinterpret_cast<std::uintptr_t>(entry_32.modBaseAddr);
                size = static_cast<std::size_t>(entry_32.modBaseSize);
                return true;
            }
...
[[nodiscard]] std::uintptr_t get_export(HANDLE handle, std::uintptr_t base, std::size_t mod_size, const char* name) {
    if (mod_size < sizeof(LONG))
        return 0;

    const auto lfanew = read_mem<LONG>(handle, base + offsetof(IMAGE_DOS_HEADER, e_lfanew));

    if (mod_size < lfanew + sizeof(IMAGE_NT_HEADERS))
        return 0;

    const auto nt = read_mem<IMAGE_NT_HEADERS>(handle, base + lfanew);
    ...
    if (mod_size < export_rva + sizeof(IMAGE_EXPORT_DIRECTORY))
        return 0;

    const auto exp = read_mem<IMAGE_EXPORT_DIRECTORY>(handle, base + export_rva);

    if (mod_size < exp.AddressOfNames + exp.NumberOfNames * sizeof(std::uint32_t))
        return 0;

    if (mod_size < exp.AddressOfNameOrdinals + exp.NumberOfNames * sizeof(std::uint16_t))
        return 0;

    if (mod_size < exp.AddressOfFunctions + exp.NumberOfNames * sizeof(std::uint32_t))
        return 0;
    ...
    for (std::uint32_t i = 0u; i < exp.NumberOfNames; i++) {
        ...
        if (mod_size < name_rvas[i] + sizeof(func_name))
            return 0;
        ...
        if (string_equals_non_case_sensitive(
            ...
            std::uintptr_t addr = base + rva;

            if (rva >= export_rva &&
                rva < export_rva + export_size) {
                char forwarder[260]{};
                if (mod_size < rva + sizeof(forwarder))
                    return 0;
                ...
                mod_name_ += ".dll";
                std::uintptr_t fwd_base;
                std::size_t fwd_size;
                const auto res = get_module(GetProcessId(handle), mod_name_.c_str(), fwd_base, fwd_size);
                if (!res)
                    return 0;

                return get_export(handle, fwd_base, fwd_size, func_part);
            }
